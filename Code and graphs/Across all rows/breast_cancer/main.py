...
# === Configuration ===
FEATURES = list(range(2, 32))  # columns 2–31 are features
LABEL_INDEX = 1                # column 1 is class (already 0/1)
...
for noise in noise_levels:
    df = pd.read_csv(os.path.join(dataset_dir, f"dataset_{noise}.csv"), header=None)
    X = df[FEATURES]
    y = df[LABEL_INDEX]
